...
try {
    ...
    elseif ($method === "POST") {
        ...
        // ตรวจสอบค่าว่าง
        if (empty($data["full_name"]) || empty($data["department"]) || empty($data["salary"]) || !isset($data["active"])) {
            echo json_encode(["success" => false, "message" => "กรุณากรอกข้อมูลให้ครบ"]);
            exit;
        }

        // ❌ เอาส่วน password ออก เพราะตาราง employees ไม่มีฟิลด์ password
        // เพิ่มข้อมูลพนักงาน
    ...
    elseif ($method === "PUT") {
        ...
        // ✅ แก้ไข: เอาเครื่องหมาย , หลัง active ออก (บรรทัดที่ 64 ของโค้ดเดิม)
